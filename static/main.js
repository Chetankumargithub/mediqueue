...
/**
 * Validate O2 saturation (70-100%)
 */
function validateO2(input) {
  const val = input.value;
  const errorEl = document.getElementById("o2Error");
  ...
  if (num < 70 || num > 100) {
    input.classList.add("error");
    errorEl.textContent = "Must be 70–100%";
    updateBtn.disabled = true;
    boostBtn.disabled = true;
    return;
  }
...
/**
 * Validate Heart Rate (30-200 bpm)
 */
function validateHR(input) {
  const val = input.value;
  ...
  if (num < 30 || num > 200) {
    input.classList.add("error");
    errorEl.textContent = "Must be 30–200 bpm";
    updateBtn.disabled = true;
    boostBtn.disabled = true;
    return;
  }
 ...
 */
function validatePain(input) {
  const val = input.value;
  const errorEl = document.getElementById("painError");
  const updateBtn = document.getElementById("btnUpdate");
  const boostBtn = document.getElementById("btnBoost");
  ...
  if (num < 1 || num > 10) {
    input.classList.add("error");
    errorEl.textContent = "Must be 1–10";
    updateBtn.disabled = true;
    boostBtn.disabled = true;
    return;
  }
